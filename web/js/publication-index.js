$(document).ready(function(){
		...
		$('input[type=image]').click(function()
		{
			...
			else
			{
				$("#formu").load("index.php?r=publication/update&id="+ID+" .publication-update", function()
				{
					...
					var lastElement = document.getElementById('year').length;
					var values = [];
					$('#year option').each(function() { values.push($(this).val()); });
					if(str.substr(0, 4) >= values[0] && str.substr(0, 4) <= values[lastElement-2])
					{
		...
		$('#deleteMulti').click(function(){
			...
			$.ajax({
			   ...
			   beforeSend: function()
			   {
				   $( "#error" ).html( '<img src="css/images/ajax-loader.gif" id="loader"/>' );
			   },
			});
